...
class SearchRequest(BaseModel):
    search_field: SearchField = Field(
        description="The field to search by: name, surname, or email"
    )
    search_value: str = Field(
        description = "The value to search for, e.g. a name, surname or email address"
    )

class SearchRequests(BaseModel):
    search_request_parameters: list[SearchRequest] = Field(
        default = [],
        description = "List of search parameters extracted from the user query"
    )
...
def retrieve_context(user_question: str) -> list[dict[str, Any]]:
    ...
    if search_requests.search_request_parameters:
        ...
        users = user_client.search(**requests_dict)
        return users
